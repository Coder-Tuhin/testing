 ...
 */
public class StructLoginMobResponse {
    ...
    private char[] charResMsg;     //ResponseMsg---CHAR---100bytes---


    public StructLoginMobResponse(byte[] data, int dataLength) {
        ...
        length = 100;
        charResMsg = new char[length];

        for (int i = 0; i < length; i++){
            charResMsg[i] = (char) data[count];
            count++;
        }
    }
    ...
    public String getResponseMsg() {
        return new String(charResMsg).trim();
    }

    public void setResponseMsg(char[] charResMsg) {
        this.charResMsg = charResMsg;
    }
