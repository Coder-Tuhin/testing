...
public class StructOCOPosnBook extends StructBase {
    ...
    public StructOCOPosnBook(byte bytes[]) {
        try {
            init();
            orgData = bytes;
            int aCount = 0;
            ...
            aCount += extract;
            posBkDetails = new StructOCOPosnDet[noOfRecs];
            iSize = new StructOCOPosnDet().data.sizeOf();//415+8
            for (int i = 0; i < noOfRecs; i++) {
                byte iBytes[] = new byte[iSize];
                System.arraycopy(bytes, aCount, iBytes, 0, iBytes.length);
                aCount += iSize;
                posBkDetails[i] = new StructOCOPosnDet(iBytes);
            }
            extract = 2;
            ep.ePacket(bytes, aCount, extract);
    ...
    public void setByteData() throws Exception {
        int aCont = 0;
        byte orgData[] = new byte[data.sizeOf() + (new StructOCOPosnDet().data.sizeOf()*noOfRecs)];

        byte clientC[] = clientCode.toBytes();
        System.arraycopy(clientC, 0, orgData, aCont, clientC.length);
        aCont += clientC.length;

        for (StructOCOPosnDet gsd : posBkDetails) {
